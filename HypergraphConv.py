...
from deterministic_scatter import deterministic_scatter_add as scatter_add
...
class HypergraphConv(MessagePassing):
    ...
    def aggregate(self, inputs, index, ptr=None, dim_size=None):
        # Overrides MessagePassing's default 'add' aggregation, which
        # otherwise dispatches to torch_scatter/PyG's internal scatter and
        # reintroduces the same CUDA nondeterminism scatter_add above avoids.
        dim = self.node_dim if self.node_dim >= 0 else inputs.dim() + self.node_dim
        return scatter_add(inputs, index, dim=dim, dim_size=dim_size)
