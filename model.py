...
class Model(nn.Module):
    ...
    def __init__(self, base_model, D_m, D_g, D_p, D_e, D_h, D_a, graph_hidden_size, n_speakers, max_seq_len, window_past, window_future, 
                 ...
                 pmr_tap=None, pmr_av_encoder='linear'):
        ...
        self.att_type = att_type
        self.last_modality_feat = None  # per-modality slices of the smax_fc input, for AMW
        # PMR: tap point for the per-modality [N, D] representation fed to the
        # prototype bank (None = PMR entirely off, forward path unchanged).
        self.pmr_tap = pmr_tap
        self.pmr_av_encoder = pmr_av_encoder
        ...
        if self.base_model == 'LSTM':
            if not self.multi_modal:
                ...
                if 'a' in self.modals:
                    hidden_a = D_g
                    self.linear_a = self._av_encoder(D_m_a, hidden_a, dropout)
                ...
                if 'v' in self.modals:
                    hidden_v = D_g
                    self.linear_v = self._av_encoder(D_m_v, hidden_v, dropout)
                    if self.av_using_lstm:
        ...
        if self.graph_type=='hyper':
            self.graph_model = HyperGCN(a_dim=D_g, v_dim=D_g, l_dim=D_g, n_dim=D_g, nlayers=64, nhidden=graph_hidden_size, nclass=n_classes,
                                        ...
                                        use_afw=use_afw, afw_rank=afw_rank, afw_beta=afw_beta, afw_nlayers=afw_nlayers, afw_dropout=afw_dropout,
                                        pmr_tap=(pmr_tap if pmr_tap in ('post_graph', 'post_afw') else None))
            print("construct "+self.graph_type)
            # Dimensionality of last_pmr_repr[m] at the configured tap, so
            # train.py can size the PrototypeBank without re-deriving it.
            _present = [m for m in ['l', 'a', 'v'] if m in self.modals]
            if self.pmr_tap == 'pre_graph':
                self.pmr_dims = {m: D_g for m in _present}
            elif self.pmr_tap in ('post_graph', 'post_afw'):
                self.pmr_dims = {m: self.graph_model.pmr_dim for m in _present}
            else:
                self.pmr_dims = None
    ...
    def _av_encoder(self, in_dim, out_dim, dropout):
        """Audio/visual modality encoder at the PMR tap-A point.

        The baseline is a single nn.Linear on frozen pre-extracted features,
        which gives PMR's PCE loss almost nothing to reshape (see the plan's
        section 1.1). With pmr_av_encoder='mlp' this becomes a 2-layer MLP of
        the same input/output dims, so PCE has non-trivial capacity to act on.
        The attribute name is unchanged (linear_a / linear_v), so AMW's
        get_modality_param_groups and the diagnostics pick the new parameters
        up automatically."""
        if self.pmr_av_encoder == 'mlp':
    ...
    def forward(self, U, qmask, umask, seq_lengths, U_a=None, U_v=None, epoch=None, collect_diagnostics=False):
        ...
        if not self.multi_modal:
            ...
            if 'l' in self.modals:
                ...
                features_l = []
        if self.pmr_tap == 'pre_graph' and self.multi_modal:
            # Last point at which each modality is purely itself: the flat,
            # dialogue-major [N, D_g] node list, in the same row order as the
            # flattened label vector built in train.py.
            # .clone() is required, not cosmetic: HyperGCN.forward adds the speaker
            # embedding to the text tensor *in place* (`l += spk_emb_vector`), which
            # would otherwise retroactively turn this tap into a post-speaker-embedding
            # view and leak PCE gradient into graph_model.speaker_embeddings --
            # breaking the fusion-independence that is the whole point of tap A.
            _tapped = {'a': features_a, 'v': features_v, 'l': features_l}
            self.last_pmr_repr = {m: _tapped[m].clone() for m in ['l', 'a', 'v'] if m in self.modals}
        ...
        if self.graph_type=='GCN3' or self.graph_type=='DeepGCN':
            ...
            emotions_feat = self.graph_model(features_a, features_v, features_l, seq_lengths, qmask, epoch, collect_diagnostics=collect_diagnostics)
            emotions_feat = self.dropout_(emotions_feat)
            emotions_feat = nn.ReLU()(emotions_feat)
            log_prob = F.log_softmax(self.smax_fc(emotions_feat), 1)

            if self.pmr_tap in ('post_graph', 'post_afw'):
                self.last_pmr_repr = self.graph_model.last_pmr_repr

            # per-modality slices of the (activated) smax_fc input, for AMW-style
            # gradient modulation -- dropout/ReLU are elementwise so slicing after
            # them matches slicing smax_fc.weight's matching columns.
            present = [m for m in ['l', 'a', 'v'] if m in self.modals]
            dims = self.graph_model.last_modality_dims
            self.last_modality_feat = {}
            offset = 0
            for m in present:
                d = dims[m]
                self.last_modality_feat[m] = emotions_feat[:, offset:offset+d]
                offset += d
        else:
            print("There are no such kind of graph")        
